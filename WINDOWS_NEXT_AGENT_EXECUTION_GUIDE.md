...
> **2026-08-01 后续状态更新：** 本文原始评估已被当前分支的后续实现部分取代。Apple Music 已按用户要求重新纳入并完成真实播放 E2E；单实例、K 歌窗视觉、发布和安装链路已闭环。最新事实以 `Windows/PROGRESS.md` 的“可用性与视觉复核”为准；下文历史 P0 指令仅用于理解当时发现的问题。
...
> Windows 端现已有可安装使用的 .NET 8 + WPF 版本。Apple Music 真实播放、歌词、罗马音/翻译、切歌、悬浮窗视觉、单实例和安装包已验收；尚未完成的是 Spotify 真实 E2E 与多显示器视觉验收。

接手后必须遵守以下总原则：

1. 保持已确认的产品范围：K 歌悬浮窗、Apple Music / Spotify 播放来源、歌词源兜底、SQLite 歌词缓存、本地英/日→中翻译、日语罗马音、托盘和精简设置。
2. Apple Music 与 Spotify 元数据处理必须隔离；Apple Music 的 `歌手 — 专辑` SMTC 规范化不得应用到 Spotify。全屏、状态栏歌词、全局快捷键、手动歌词搜索、自动更新、多语言 UI 仍不在当前范围。
