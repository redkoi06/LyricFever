...
enum SpotifyLyricError: LocalizedError {
    case isLocalFile
    case tooManyTries
    case unauthorized
    ...
    var errorDescription: String? {
        switch self {
        case .isLocalFile:
            return "Spotify local files do not provide network lyrics."
        case .tooManyTries:
            return "Spotify rate limited the lyric request."
        case .unauthorized:
            return "Spotify authorization expired."
        case .invalidResponse:
            return "Spotify returned an invalid response."
        case .httpStatus(let status):
            return "Spotify request failed with HTTP status \(status)."
        }
    }
}

enum AccessTokenError: LocalizedError {
    case badSecret
    case badURL
    case unauthorized
    case invalidResponse
    ...
    var errorDescription: String? {
        switch self {
        case .badSecret:
            return "Spotify authentication secret is invalid."
        case .badURL:
            return "Spotify authentication URL is invalid."
        case .unauthorized:
            return "Spotify login expired. Please sign in again."
        case .invalidResponse:
            return "Spotify returned an invalid authentication response."
        case .httpStatus(let status):
            return "Spotify authentication failed with HTTP status \(status)."
        }
    }
}
...
class SpotifyLyricProvider: LyricProvider {
    ...
    @MainActor
    private func authenticatedData(for request: URLRequest, session: URLSession) async throws -> Data {
        let (data, response) = try await session.data(for: request)
        guard let response = response as? HTTPURLResponse else {
            throw AccessTokenError.invalidResponse
        }
        if response.statusCode == 401 {
            throw AccessTokenError.unauthorized
        }
        guard (200..<300).contains(response.statusCode) else {
            throw AccessTokenError.httpStatus(response.statusCode)
        }
        return data
    }
    ...
    var secretData: [Int] {
        get async throws {
            ...
            let data = try await authenticatedData(for: URLRequest(url: url), session: .shared)
...
//            }
    ...
    func generateAccessToken() async throws {
        ...
        if !isAccessTokenAlive {
                ...
                let serverTimeRequest = URLRequest(url: serverTimeURL)
                let serverTimeData = try await authenticatedData(for: serverTimeRequest, session: fakeSpotifyUserAgentSession)
                ...
                let accessTokenData: Data
                do {
                    accessTokenData = try await authenticatedData(for: request, session: fakeSpotifyUserAgentSession)
                } catch AccessTokenError.unauthorized {
                    accessToken = nil
                    UserDefaults.standard.set(false, forKey: "hasOnboarded")
                    throw AccessTokenError.unauthorized
                }
                do {
                    accessToken = try JSONDecoder().decode(AccessTokenJSON.self, from: accessTokenData)
                } catch {
                    accessToken = nil
                    if let errorWrap = try? JSONDecoder().decode(ErrorWrapper.self, from: accessTokenData),
                       errorWrap.error.code == 401 {
                        UserDefaults.standard.set(false, forKey: "hasOnboarded")
                        throw AccessTokenError.unauthorized
                    }
                    throw AccessTokenError.invalidResponse
...
//        try? await Task.sleep(for: .seconds(2))
        ...
        if let accessToken, let url = URL(string: "https://spclient.wg.spotify.com/color-lyrics/v2/track/\(trackID)?format=json&vocalRemoval=false") {
            ...
            guard let response = urlResponseAndData.1 as? HTTPURLResponse else {
                throw SpotifyLyricError.invalidResponse
            }
            if response.statusCode == 401 {
                self.accessToken = nil
                throw SpotifyLyricError.unauthorized
            }
            if response.statusCode == 429 {
                throw SpotifyLyricError.tooManyTries
            }
            guard (200..<300).contains(response.statusCode) else {
                throw SpotifyLyricError.httpStatus(response.statusCode)
            }
    ...
    func searchForTrackForAppleMusic(artist: String, track: String, album: String? = nil) async throws -> AppleMusicHelper? {
        ...
        if let url = URL(string: "https://api-partner.spotify.com/pathfinder/v2/query") {
            ...
            guard let response = searchData.1 as? HTTPURLResponse else {
                throw SpotifyLyricError.invalidResponse
            }
            if response.statusCode == 401 {
                self.accessToken = nil
